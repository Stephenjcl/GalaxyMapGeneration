...
from constants import CM_PER_KM, CM_PER_METER, SECONDS_PER_HOUR, DAYS_IN_A_YEAR, RADIANS_PER_ROTATION, MILLIBARS_PER_BAR, MMHG_TO_MILLIBARS
...
def kothari_radius(mass, giant, zone):
    ...
    temp = (2.0 * BETA_20 * (SOLAR_MASS_IN_GRAMS ** (1.0 / 3.0))) / (A1_20 * (temp1 ** (1.0 / 3.0)))

    temp2 = A2_20 * (atomic_weight ** (4.0 / 3.0)) * (SOLAR_MASS_IN_GRAMS ** (2.0 / 3.0))
...
def empirical_density(mass, orb_radius, r_ecosphere, gas_giant):
    ...
    temp = ((mass * SUN_MASS_IN_EARTH_MASSES) ** (1.0 / 8.0)) * ((r_ecosphere / orb_radius) ** (1.0 / 4.0))
    if gas_giant:
        return(temp * 1.2)
    else:
        return(temp * 5.5)
...
def day_length(planet):
    ...
    try:
        base_angular_velocity = sqrt(2.0 * J * (planetary_mass_in_grams) / (k2 * (equatorial_radius_in_cm ** 2)))
    except TypeError:
    ...
    if ang_velocity.real <= 0.0 or isinstance(ang_velocity, complex):
        stopped = True
...
def pressure(volatile_gas_inventory, equat_radius, gravity):
    ...
    pres_return = volatile_gas_inventory * gravity * (EARTH_SURF_PRES_IN_MILLIBARS / 1000.) / (equat_radius ** 2)
    if isinstance(pres_return, complex):
        pres_return = 0.0
    return pres_return
...
def opacity(molecular_weight, surf_pressure):
    ...
    if isinstance(surf_pressure, complex):
        surf_pressure = 0.0
...
def breathability(planet):
    ...
    #if planet.gases == 0:
    if planet.atmosphere is None or planet.gases == 0:
    ...
    for index in range(planet.gases):
        ...
        ipp = inspired_partial_pressure(planet.surf_pressure,
                                        planet.atmosphere[index][1])  # [1] was .surf_pressure

        for n in range(planet.gases):  # range(len(planet.gases)):
            # if planet.gases[n].num == planet.atmosphere[index].num:
            if gases[n].symbol == planet.atmosphere[index][0]:
                gas_no = n
        ...
        # for some reason oxygen checks aren't working. All planets are coming through
        # as unbreathable.
            # if ((ipp >= MIN_O2_IPP) and (ipp <= MAX_O2_IPP))
        # if gases[gas_no].max_ipp == 400 * MMHG_TO_MILLIBARS:
            # oxygen_ok = True

    # Build a really dumb hack to get the oxygen check to work.
    atmosphere_string = tabulate([[gas.symbol, str(amount) + ' mb'] for gas, amount in planet.atmosphere])

    # if oxygen_ok is True:
    # if "O" in atmosphere_string:
    if re.search(r'\bO\b', atmosphere_string):
