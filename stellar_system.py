...
import attr
from enviroment import PlanetType, BreathabilityPhrase
...
class Planet():
    ...
    mapname = attrib(default = "Unknown")

    breath = attrib(default=BreathabilityPhrase.NONE)
    #   ZEROES end here

    def __repr__(self):
        if self.atmosphere is None:
            atmosphere_string = "No Breathable Atmosphere"
        else:
        ...
        return tabulate([
            ...
            ['Radius', str("%.2f" % round(self.radius,2)) + " km"],
            ['Orbit', self.orbit],
            ['Surface gravity', str(self.surf_grav) + ' g'],
            ['Surface pressure', str(self.surf_pressure / MILLIBARS_PER_ATM) + ' atm'],
            ['Surface temperature', str(int(round(self.min_temp))) + "-" + str(int(round(self.max_temp))) + ' K'],
            ['Atmosphere', atmosphere_string],
            ['Breathability', self.breath],
